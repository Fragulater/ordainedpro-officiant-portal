...
SET status = CASE
  ...
  WHEN status IN ('pending', 'accepted', 'declined', 'confirmed', 'canceled', 'completed') THEN status
  ELSE 'pending'
END
WHERE status IS NULL
  OR status NOT IN ('pending', 'accepted', 'declined', 'confirmed', 'canceled', 'completed');
