...
export function CommunicationPortal({ onScriptUploaded, user: initialUser }: CommunicationPortalProps = {}) {
  ...
  const loadTasksForCouple = useCallback(async () => {
    ...
    console.log("ðŸ“‹ Loading tasks for couple:", editCoupleInfo.id)
    ...
    if (result.ok && result.data) {
      ...
      console.log("ðŸ“­ No tasks found or error for couple", editCoupleInfo.id)
      setTasks([])
    }
  ...
  const loadFilesForCouple = useCallback(async () => {
    ...
    console.log("ðŸ“ Loading files for couple:", editCoupleInfo.id)
    ...
    if (result.ok && result.data) {
      ...
      console.log("ðŸ“­ No files found or error for couple", editCoupleInfo.id)
      setFiles([])
    }
  ...
  const loadMeetingsForCouple = useCallback(async () => {
    ...
    console.log("ðŸ“… Loading meetings for couple:", editCoupleInfo.id)
    ...
    if (result.ok && result.data) {
      ...
      console.log("ðŸ“­ No meetings found or error for couple", editCoupleInfo.id)
      setMeetings([])
    }
  ...
  const loadContractsForCouple = useCallback(async () => {
    ...
    console.log("ðŸ“œ Loading contracts for couple:", editCoupleInfo.id)
    ...
    if (result.ok && result.data) {
      ...
      console.log("ðŸ“­ No contracts found or error for couple", editCoupleInfo.id)
      setContracts([])
    }
  ...
  const loadPaymentsForCouple = useCallback(async () => {
    ...
    if (result.ok && result.data) {
      ...
      console.log("ðŸ“­ No payments found or error for couple", editCoupleInfo.id)
      setPaymentHistory([])
      setPaymentInfo({
        totalAmount: 0,
        depositPaid: 0,
  ...
  const loadScriptsForUser = useCallback(async () => {
    ...
    console.log("ðŸ“œ Loading scripts for user:", currentUser.id)
    ...
    if (result.ok && result.data) {
      ...
      console.log("ðŸ“­ No scripts found or error")
      ...
      if (currentContent.trim()) {
        ...
        if (editingScript.id && typeof editingScript.id === 'number') {
          ...
          if (result.ok) {
            ...
            alert(`âš ï¸ Failed to auto-save to server. Please try again.`)
          }
        } else {
...
Pastor Michael Adams`,
  ...
  const handleSaveScript = async () => {
    ...
    try {
      ...
      alert(`âš ï¸ Failed to save script to server.\n\nError: ${err.message}\n\nPlease try again.`)
    }
  }
...
ðŸ‘°ðŸ¤µ For: ${editCoupleInfo?.brideName || 'Partner 1'} & ${editCoupleInfo?.groomName || 'Partner 2'}
...
pastor.michael@ordainedpro.com`
  ...
  const handleContractUploaded = async (contractData: Omit<Contract, 'id' | 'createdDate'>) => {
    ...
    console.log("ðŸ“œ Uploading contract for couple:", editCoupleInfo.id)
...
Note: This is an initial draft. Further development needed to incorporate specific cultural and religious elements from both Sarah's and David's backgrounds.`
  ...
  const handleAddTask = async (newTaskData: Omit<Task, 'id' | 'createdDate'>) => {
    ...
    console.log("ðŸ“‹ Adding task for couple:", editCoupleInfo.id)
  ...
  const scheduleEmailNotification = async (task: Task) => {
    ...
    console.log(`ðŸ“… Reminder scheduled for ${reminderDate.toDateString()} - Recipients: ${recipients.join(', ')}`)
  }
  ...
  const generateTaskReminderEmail = (task: Task, isOfficiant: boolean, coupleName: string, officiantName: string) => {
    ...
    return {
      subject: `ðŸ“‹ Wedding Task: ${task.task}`,
      body: `${greeting},
...
ðŸ“‹ Task: ${task.task}
ðŸ“… Due Date: ${new Date(task.dueDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
â° Due Time: ${task.dueTime}
${priorityEmoji[task.priority] || 'ðŸ“Œ'} Priority: ${task.priority.charAt(0).toUpperCase() + task.priority.slice(1)}
ðŸ“ Category: ${task.category}

${task.details ? `ðŸ“ Details:\n${task.details}` : ''}
...
`
  ...
  const handleScheduleMeeting = async (meetingData: Omit<Meeting, 'id' | 'createdDate' | 'status' | 'reminderSent' | 'calendarInviteSent'>) => {
    ...
    console.log("ðŸ“… Scheduling meeting for couple:", editCoupleInfo.id)
  ...
  const handleFilesUploaded = async (uploadedFiles: UploadedFile[]) => {
    ...
    console.log("ðŸ“ Uploading", uploadedFiles.length, "files for couple:", editCoupleInfo.id)
...
${invoiceContent}`)
  ...
  // Always wrap in provider to avoid bundling issues with production builds
  return (
